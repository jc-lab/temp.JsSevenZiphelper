...
namespace JsCPPUtils
{
...
#ifdef _UNICODE
			m_fileFullPath = std::basic_string<wchar_t>(filenameInfo.FileName, filenameInfo.FileNameLength);
#else
	...
	bool FileMaker::create(const char *szFilePath, bool bNoOverwrite, bool isDirectory)
	{
		...
		if (isDirectory)
		{
			if (CreateDirectoryA(szRealPath, NULL))
			{
				m_hFile = CreateFileA(szRealPath, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS, NULL);
			}
			else {
				m_dwErr = GetLastError();
				retval = false;
			...
			m_hFile = CreateFileA(szRealPath, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ, NULL, bNoOverwrite ? CREATE_NEW : CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
		}
	...
	bool FileMaker::create(const wchar_t *szFilePath, bool bNoOverwrite, bool isDirectory)
	{
		bool retval;
		wchar_t szRealPath[MAX_PATH] = { 0 };

		GetFullPathNameW(szFilePath, MAX_PATH, szRealPath, NULL);

		if (isDirectory)
		{
			if (CreateDirectoryW(szRealPath, NULL))
			{
				m_hFile = CreateFileW(szRealPath, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS, NULL);
			}
			else {
				m_dwErr = GetLastError();
				retval = false;
			...
			m_hFile = CreateFileW(szRealPath, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ, NULL, bNoOverwrite ? CREATE_NEW : CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
		}
		if (m_hFile == INVALID_HANDLE_VALUE)
		{
			m_dwErr = GetLastError();
	...
	/*
	std::basic_string<TCHAR> FileMaker::getFullFilePath()
	{
